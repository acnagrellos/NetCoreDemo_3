...
app.MapPut("/contacts/{id}", async (HttpContext http, int id, Contact contactRequest, IContactService contactService) =>
{
    try 
    {
...
app.MapPost("/contacts", (Contact contact, IContactService contactService) => 
    contactService.CreateContact(contact.Id, contact.Name, contact.TelephoneNumber));
